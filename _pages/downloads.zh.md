...
lang: zh
translation_url: /downloads/
author_profile: true
comments: false
share: false
---

这里集中放置博客教程配套的代码模板与示例文件。下载后先解压<span class="download-access-password">（访问密码：NOOB304）</span>，再按照对应博客中的说明配置环境、替换自己的数据并运行脚本。

如果只是想复现教程，建议优先下载对应文章旁边的代码包；如果是正式论文分析，请先用小范围数据测试，再扩展到完整研究区。
...
| LandTrendr/GEE 遥感长时间序列扰动与恢复识别 | [下载 landtrendr_ecological_change_template.zip](/assets/downloads/landtrendr_ecological_change_template.zip){: .btn .btn--primary} | [阅读教程](/posts/2026/06/landtrendr-gee-ecological-disturbance-recovery/) |
