...
点位或表格样本可用于训练回归模型，训练好的模型再应用到 GeoTIFF 栅格，生成连续空间预测图。

原始代码包的文件名以 `GPP` 命名，但方法并不只适用于 GPP。只要研究对象是连续型数值变量，并且同时有样本点表格和对应的栅格解释变量，就可以套用这套框架。例如：GPP、NPP、植被覆盖度、LAI、NDVI、土壤有机碳、碳储量、地表温度、水质指标、生态质量指数等，都可以按同样思路处理。
...
训练表格和栅格文件必须一一对应。表格里用 `X1` 到 `X5`，栅格预测时就必须有 `x1_年月.tif` 到 `x5_年月.tif`。
...
Python、ArcGIS、QGIS 或 R 都能完成同样的整理。最终文件名、变量顺序和空间位置必须一致。
