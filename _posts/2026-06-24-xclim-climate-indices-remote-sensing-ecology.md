...
遥感生态论文经常使用年均温、年降水量或月尺度气候均值解释 NDVI、NPP、GPP 和生态系统服务变化。连续高温、极端干旱、强降水、热量积累、生长季长度、降水集中度和霜冻日数等指标能补充均值变量没有表达的阈值、持续时间与极端事件。

xclim 是一个基于 xarray 的 Python 气候指标库，可以把 NetCDF 气候数据转化为大量标准化气候指标，并支持 dask 并行处理、单位检查、元数据处理、偏差订正和集合分析。它可以将 CMIP6 或 ERA5 数据转换为生态过程相关指标，再输入机器学习、因果分析、碳汇预测或生态风险评估模型。
...
可以先把 CMIP6 温度、降水转化为连续干旱天数、强降水频率和热浪日数，再用于未来生态系统风险评估。这些指标比原始气候均值更接近具体的生态胁迫过程。
...
xclim 可以把基础气候变量整理成与阈值、累积、持续时间和极端事件有关的指标，为后续归因、预测和风险评估提供输入。
