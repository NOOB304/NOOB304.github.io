---
title: "从 0 入门：用机器学习把站点 GPP 数据预测成月尺度栅格图"
date: 2026-06-27
permalink: /posts/2026/06/gpp-machine-learning-raster-prediction-tutorial/
lang: zh
author_profile: true
comments: false
share: false
tags:
  ...
  - 栅格预测
  - 遥感
excerpt: "基于一个 GPP 建模代码包，完整拆解站点 CSV、模型训练、参数寻优、精度评价、GeoTIFF 批量预测和结果检查流程。"
---

这篇文章整理一个比较完整的 GPP 机器学习建模流程：先用站点尺度样本训练模型，再把训练好的模型应用到月尺度 GeoTIFF 栅格上，得到连续空间上的 GPP 预测图。

原始代码包里包含六类机器学习模型：随机森林、XGBoost、LightGBM、CatBoost、支持向量机和高斯过程回归。它们的整体逻辑高度相似：读取站点表格、划分训练集和测试集、网格搜索最优参数、保存模型和评价指标，最后再用对应的栅格预测脚本批量输出每个月的 GPP 图。
...
```text
用已有站点样本学习 Y 与 X1-X5 的关系，然后把这个关系推广到每一个栅格像元。
```

在这套代码中，站点数据表叫 `datasite.csv`，它的列结构是：
...
也就是说，每一个月份都需要 5 个输入栅格，分别对应训练表里的 `X1` 到 `X5`。模型会对每个像元读取这 5 个值，预测该像元的 GPP，然后输出：
...
| 文件或文件夹 | 作用 |
|---|---|
| `datasite.csv` | 站点样本表，包含 `Month`、`Y`、`X1` 到 `X5` |
| `GPP_RF.py` | 训练随机森林模型 |
...
| `重命名数据.m` | MATLAB 预处理脚本，用来把不同来源的 GPP 栅格统一命名为 `x1_YYYYMM.tif` 到 `x5_YYYYMM.tif` |

从 MATLAB 脚本可以看出，这套数据的 5 个输入栅格大概来自 5 套 GPP 产品或数据源：
...
```bash
conda create -n gpp python=3.10
conda activate gpp
...
```python
D:/Project-GPP/SiteData/datasite.csv
D:/Project-GPP/grid_datasite/ALL/
```
...
└─ scripts
   ├─ 01_train_rf.py
   └─ 02_predict_raster_rf.py
```

其中最重要的是两个输入位置：

- `SiteData/datasite.csv`：站点训练表；
- `grid_datasite/ALL/`：准备好的逐月输入栅格。

## 5. 准备站点表格 `datasite.csv`

站点表格必须是 CSV 格式，不能直接用 Excel 的 `.xlsx`。如果你现在手里是 Excel 文件，可以在 Excel 里另存为 CSV。
...
为什么要有 `Month`？因为 GPP 有明显季节性。如果随机划分训练集和测试集时不管月份，可能某些月份在测试集中太少，评价结果不稳定。原代码使用了：
...
这里的 `stratify=months` 就是按月份分层抽样，让 1 月到 12 月的样本比例尽量保持一致。

## 6. 准备栅格输入文件

栅格预测阶段要求每个月都有 5 个输入文件。以 2000 年 1 月为例：
...
```text
读取 MODIS、PML、CEDAR、GOSIF、TL-LUE 的每月 GPP
  ↓
转换成 double 数组
  ↓
按统一投影参考写出
...
如果你不使用 MATLAB，也可以用 Python、ArcGIS、QGIS 或 R 完成同样的事情。重点不是工具，而是最终文件名和空间对齐必须正确。

## 7. 训练随机森林模型

先从随机森林开始，因为它对数据尺度不太敏感，不需要像 SVM 那样强依赖标准化，适合做入门版本。

在 `scripts` 文件夹中新建 `01_train_rf.py`：
...
# 1. 项目路径
# 如果你的项目不在 D:/Project-GPP，需要改这里
PROJECT_DIR = Path(r"D:/Project-GPP")
...
with open(OUT_DIR / "rf_model_metrics.json", "w", encoding="utf-8") as f:
    json.dump(metrics, f, ensure_ascii=False, indent=2)

X_train.to_csv(OUT_DIR / "rf_X_train.csv", index=False)
X_test.to_csv(OUT_DIR / "rf_X_test.csv", index=False)
y_train.to_frame("Y").to_csv(OUT_DIR / "rf_y_train.csv", index=False)
y_test.to_frame("Y").to_csv(OUT_DIR / "rf_y_test.csv", index=False)

pd.DataFrame({"y_pred_train": y_pred_train}).to_csv(
    OUT_DIR / "rf_y_pred_train.csv",
    index=False,
)
pd.DataFrame({"y_pred_test": y_pred_test}).to_csv(
    OUT_DIR / "rf_y_pred_test.csv",
    index=False,
)

joblib.dump(best_model, OUT_DIR / "trained_random_forest_model.pkl")
...
| `rf_model_metrics.json` | 训练集和测试集评价指标 |
| `trained_random_forest_model.pkl` | 保存好的随机森林模型 |
| `rf_X_train.csv`、`rf_X_test.csv` | 划分后的自变量 |
| `rf_y_train.csv`、`rf_y_test.csv` | 划分后的真实值 |
| `rf_y_pred_train.csv`、`rf_y_pred_test.csv` | 模型预测值 |
...
```text
读取 trained_random_forest_model.pkl
  ↓
循环 year = 2000 到 2020
  ↓
循环 month = 1 到 12
...
在 `scripts` 文件夹中新建 `02_predict_raster_rf.py`：
...
# 1. 路径设置
PROJECT_DIR = Path(r"D:/Project-GPP")
MODEL_FILE = PROJECT_DIR / "SiteData" / "ML_RF" / "trained_random_forest_model.pkl"
...
def predict_one_month(model, year, month):
    ...
    output_file = OUTPUT_DIR / f"GPP_{year}_{month:02d}.tif"
...
```bash
python scripts/02_predict_raster_rf.py
```
...
joblib.dump(best_model, OUT_DIR / "trained_catboost_model.pkl")
```

栅格预测脚本中对应改成：

```python
MODEL_FILE = PROJECT_DIR / "SiteData" / "ML_CAT" / "trained_catboost_model.pkl"
...
```text
FileNotFoundError: D:\Project-GPP\SiteData\datasite.csv
```
...
1. 在 QGIS 或 ArcGIS 中打开几个输出 `GPP_YYYY_MM.tif`；
2. 检查坐标系是否正确；
3. 检查空间范围是否和输入栅格一致；
4. 检查 NoData 区域是否合理；
5. 检查数值范围是否明显异常；
...
3. 输入栅格
   - D:\Project-GPP\grid_datasite\ALL\x1_200001.tif
   - D:\Project-GPP\grid_datasite\ALL\x2_200001.tif
   - ...
   - 一直到 x5_202012.tif

4. 训练脚本
   - scripts\01_train_rf.py

5. 预测脚本
   - scripts\02_predict_raster_rf.py

6. 输出结果
   - SiteData\ML_RF\trained_random_forest_model.pkl
   - SiteData\ML_RF\rf_model_metrics.json
   - grid_datasite\Result_RF\GPP_YYYY_MM.tif
```

这套流程的核心思想其实很朴素：表格负责学习关系，栅格负责提供空间位置。只要训练表中的 `X1-X5` 和栅格中的 `x1-x5` 含义一致，机器学习模型就可以把站点尺度的关系推广到整个研究区。
