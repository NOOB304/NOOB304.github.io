...
两期遥感影像只能说明起点与终点不同，却很难回答变化发生在哪一年、持续多久、是突发扰动还是缓慢退化，以及植被后来是否恢复。LandTrendr 将年度 Landsat 光谱序列拟合为若干分段直线，可从断点和变化段中提取扰动年份、幅度、持续时间和恢复速率。配套的 Google Earth Engine 单文件脚本涵盖 Landsat 5/7/8/9 预处理、年度 NBR 合成、扰动与恢复筛选、最小斑块过滤、结果导出和本地面积统计。
...
LandTrendr 输出扰动年份、持续时间、幅度和恢复过程，这些时间信息可用于后续的样本划分、事件窗口和因果设计。

变化原因需要结合年度影像质量、参数敏感性、参考数据、气候、人类活动和工程边界判断。
