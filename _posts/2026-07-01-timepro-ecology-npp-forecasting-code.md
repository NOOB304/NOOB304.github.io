...
[下载timepro_ecology_forecast.py](/assets/downloads/timepro_ecology_forecast.py){: .btn .btn--primary}

[下载依赖清单](/assets/downloads/timepro_ecology_requirements.txt)

这里提供的是一个普通 Python 源文件，不是 EXE、GUI、安装程序或带批处理入口的工程包。训练、自动调参、70%/30%站点划分、M0/M1/M2比较、模型保存和可选的GeoTIFF预测都在同一个 `.py` 文件中。

采用推荐CSV列名时，通常只需要修改 `STATION_CSV` 和 `OUTPUT_DIR` 两个路径。月尺度与年尺度通过 `TIME_SCALE` 切换，不再读取YAML配置，也不需要运行 `.bat` 文件。
...
这些是开发阶段用模拟数据做的流程检查。现在下载的单文件代码不会在找不到CSV时偷偷生成模拟数据，而是直接提示路径错误，避免初学者误把演示数据当成自己的结果。
...
如果某站点只有2010、2011、2015年，2011和2015不能当作相邻年份。这次单文件版已经加入日期间隔检查：
...
timepro_ecology_requirements.txt
```

安装依赖：

```powershell
python -m pip install -r timepro_ecology_requirements.txt
```

打开 `timepro_ecology_forecast.py`，最少只修改两个路径：
...
此时 `FEATURE_COLUMNS = []` 即可自动把 A、B、C、D 识别为 X。如果列名不同，再修改：
