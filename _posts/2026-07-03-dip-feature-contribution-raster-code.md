...
下面先拆解原论文，再把作者面向表格数据的方法改成栅格代码。代码可以读入任意研究区的多变量 GeoTIFF，输出研究区总体 DIP 表格、预测栅格、逐变量贡献栅格、百分比栅格和两两交互栅格。
...
论文讨论的是 global loss-based feature attribution，也就是用预测损失衡量变量在整个数据集上的贡献。
...
普通排名没有显示这些组成部分。一个变量最终分数小，可能是自身预测信息少，也可能是其他变量已经提供了相似信息。较大的最终分数也可能主要来自与其他变量的配合。
...
Example 3 中，DGP1 没有交互，也没有变量依赖。DGP2 同时存在交互和依赖，但两者对预测能力的影响正好抵消。两组数据用普通 value function 或重要性分数看起来相同，DIP 可以把它们分开。
...
DIP 衡量变量包含的预测信息。若要估计改变 X 后 Y 会怎样变化，还需要因果识别设计。
...
观测栅格得到的是预测贡献，论文中可使用“预测信息”“关联”或“响应”等词。
